#ifndef _BOARD_CONFIG_H_
#define _BOARD_CONFIG_H_

#include <driver/gpio.h>

#define AUDIO_INPUT_SAMPLE_RATE  24000
#define AUDIO_OUTPUT_SAMPLE_RATE 24000
  
//ES8311
#define AUDIO_I2S_GPIO_MCLK GPIO_NUM_21
#define AUDIO_I2S_GPIO_WS   GPIO_NUM_11
#define AUDIO_I2S_GPIO_BCLK GPIO_NUM_13
#define AUDIO_I2S_GPIO_DIN  GPIO_NUM_12
#define AUDIO_I2S_GPIO_DOUT GPIO_NUM_10
#define AUDIO_CODEC_PA_PIN  GPIO_NUM_46    //定义音频编解码器的功率放大器（PA）引脚

#define AUDIO_CODEC_I2C_SDA_PIN  GPIO_NUM_14
#define AUDIO_CODEC_I2C_SCL_PIN  GPIO_NUM_45
#define AUDIO_CODEC_ES8311_ADDR  ES8311_CODEC_DEFAULT_ADDR

#define VOLUME_UP_BUTTON_GPIO   GPIO_NUM_NC
#define VOLUME_DOWN_BUTTON_GPIO GPIO_NUM_NC

//分辨率
#define DISPLAY_WIDTH   160
#define DISPLAY_HEIGHT  160
//镜像和轴交换：通常不需要镜像或轴交换，除非你的硬件设计需要特定的显示方向。
#define DISPLAY_MIRROR_X true
#define DISPLAY_MIRROR_Y false
#define DISPLAY_SWAP_XY false
//默认不启用颜色反转。
#define DISPLAY_INVERT_COLOR    true
//RGB顺序：GC9D01N使用RGB顺序
#define DISPLAY_RGB_ORDER  LCD_RGB_ELEMENT_ORDER_RGB
//偏移量：通常不需要偏移，除非你的显示屏有特定的显示区域限制。
#define DISPLAY_OFFSET_X  0
#define DISPLAY_OFFSET_Y  0
//背光输出取反：默认不取反，除非你的背光控制电路需要取反信号。
#define DISPLAY_BACKLIGHT_OUTPUT_INVERT true
#define DISPLAY_BACKLIGHT_PIN GPIO_NUM_NC
#define DISPLAY_SPI_SCLK_HZ     (40 * 1000 * 1000)

/*==========小智+魔眼============ */
/* LCD size */
#define GC9A01_LCD_H_RES   (160)
#define GC9A01_LCD_V_RES   (160)
/* LCD settings */
#define GC9A01_LCD_SPI1_NUM         (SPI3_HOST)
#define GC9A01_LCD_SPI2_NUM         (SPI2_HOST)
#define GC9A01_LCD_PIXEL_CLK_HZ    (20 * 1000 * 1000)
#define GC9A01_LCD_CMD_BITS        (8)
#define GC9A01_LCD_PARAM_BITS      (8)
#define GC9A01_LCD_COLOR_SPACE     (ESP_LCD_COLOR_SPACE_RGB)
#define GC9A01_LCD_BITS_PER_PIXEL  (16)
#define GC9A01_LCD_DRAW_BUFF_DOUBLE (1)
#define GC9A01_LCD_DRAW_BUFF_HEIGHT (160)
#define GC9A01_LCD_BL_ON_LEVEL     (1)
/* LCD-SPI2 pins - 已移除，改为触摸传感器和按钮功能 */

/* 触摸传感器配置 */
#define TOUCH_SENSOR_GPIO               (GPIO_NUM_0)   // 触摸传感器1（播放 SUCCESS）- 使用 GPIO 0
#define TOUCH_SENSOR_CHANNEL            (1)            // 触摸通道 1 (通道 0 是内部降噪通道，不能使用)
#define TOUCH_SENSOR2_GPIO              (GPIO_NUM_2)   // 触摸传感器2（打断 AI）- 使用 GPIO 2
#define TOUCH_SENSOR2_CHANNEL           (2)            // 触摸通道 2

// 百分比阈值（参考项目使用 0.15 = 15%，0.4 = 40%）
#define TOUCH_THRESHOLD_PERCENT          0.15f          // 轻触阈值（15% 变化率）
#define TOUCH_HEAVY_THRESHOLD_PERCENT    0.40f          // 重触阈值（40% 变化率）

/* LCD-SPI1 pins - 正确的硬件连接 */
#define GC9A01_SPI1_LCD_GPIO_SCLK       (GPIO_NUM_38)  // SCL
#define GC9A01_SPI1_LCD_GPIO_MOSI       (GPIO_NUM_39)  // SDA
#define GC9A01_SPI1_LCD_GPIO_RST        (GPIO_NUM_40)  // RST
#define GC9A01_SPI1_LCD_GPIO_DC         (GPIO_NUM_0)   // DC
#define GC9A01_SPI1_LCD_GPIO_CS         (GPIO_NUM_NC)
#define GC9A01_SPI1_LCD_GPIO_BL         (GPIO_NUM_NC)
#define GC9A01_SPI1_LCD_GPIO_MISO       (GPIO_NUM_NC)

/*#define LCD_TYPE_GC9A01_SERIAL
#define DISPLAY_WIDTH   240
#define DISPLAY_HEIGHT  240
#define DISPLAY_MIRROR_X true
#define DISPLAY_MIRROR_Y false
#define DISPLAY_SWAP_XY false
#define DISPLAY_INVERT_COLOR    true
#define DISPLAY_RGB_ORDER  LCD_RGB_ELEMENT_ORDER_BGR
#define DISPLAY_OFFSET_X  0
#define DISPLAY_OFFSET_Y  0
//#define DISPLAY_BACKLIGHT_OUTPUT_INVERT false
#define DISPLAY_SPI_MODE 0*/

#define BOOT_BUTTON_GPIO        GPIO_NUM_NC

//Camera Config
/* Camera pins */
#define CAMERA_PIN_PWDN -1
#define CAMERA_PIN_RESET -1
#define CAMERA_PIN_XCLK GPIO_NUM_16
#define CAMERA_PIN_SIOD GPIO_NUM_4
#define CAMERA_PIN_SIOC GPIO_NUM_5

#define CAMERA_PIN_D7 GPIO_NUM_15
#define CAMERA_PIN_D6 GPIO_NUM_17
#define CAMERA_PIN_D5 GPIO_NUM_18
#define CAMERA_PIN_D4 GPIO_NUM_47
#define CAMERA_PIN_D3 GPIO_NUM_44
#define CAMERA_PIN_D2 GPIO_NUM_3
#define CAMERA_PIN_D1 GPIO_NUM_9
#define CAMERA_PIN_D0 GPIO_NUM_48
#define CAMERA_PIN_VSYNC GPIO_NUM_6
#define CAMERA_PIN_HREF GPIO_NUM_7
#define CAMERA_PIN_PCLK GPIO_NUM_8

//#define XCLK_FREQ_HZ 10000000
#define XCLK_FREQ_HZ 20000000




#endif // _BOARD_CONFIG_H_
