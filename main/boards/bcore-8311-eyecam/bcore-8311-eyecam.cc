...
private:
    ...
    Button boot_button_;
    LcdDisplay* display_;
    TouchButtonManager* touch_button_manager_;
     /* LCD IO and panel */
...
void InitializeI2c2() {
    ...
    //初始化单屏（只使用 SPI3_HOST）
    void InitializeSingleScreenEye(){
        // 只初始化第一块屏幕（SPI3_HOST）
        InitializeSpiEye1();
        InitializeGc9d01nDisplayEye1();

        // 创建单屏显示对象（第二块屏幕传 NULL）
        DisplayFonts fonts = {
        ...
        display_ = new DualScreenDisplay(
            lcd_io1, lcd_panel1,    // 第一块屏幕（SPI3_HOST）
            NULL, NULL,              // 第二块屏幕不使用
    ...
    //初始化触摸传感器和按钮
    void InitializeTouchAndButtons() {
        touch_button_manager_ = new TouchButtonManager();
        touch_button_manager_->Initialize();

        auto& app = Application::GetInstance();

        // 触摸传感器 (GPIO 1) → 播放 SUCCESS 音效
        touch_button_manager_->SetTouchSensorCallback([&app]() {
            ESP_LOGI(TAG, "GPIO %d touched - Playing SUCCESS", TOUCH_SENSOR_GPIO);
            app.PlaySound(Lang::Sounds::OGG_SUCCESS);
        });

        // 触摸传感器 (GPIO 2) → 打断 AI 语音
        touch_button_manager_->SetTouchSensor2Callback([&app]() {
            ESP_LOGI(TAG, "GPIO %d touched - Aborting AI speech", TOUCH_SENSOR2_GPIO);
            if (app.GetDeviceState() == kDeviceStateSpeaking) {
                app.AbortSpeaking(kAbortReasonNone);
            } else {
                ESP_LOGI(TAG, "Not speaking, nothing to abort");
            }
        });
    }
...
public:
  ...
  XiaoZhiEyeBoard() : boot_button_(BOOT_BUTTON_GPIO) {
    ...
    InitializeI2c();
     InitializeI2c2();
ESP_LOGI(TAG, "Init single eye");
ESP_LOGD(TAG, "Init single eye");
    lcd_mutex_init();
    InitializeSingleScreenEye();  // 改为单屏初始化
    InitializeTouchAndButtons();   // 重新启用（已优化 CPU 占用）
    ESP_LOGI(TAG, "Touch sensors enabled (optimized: 5Hz scan, threshold=150)");
ESP_LOGI(TAG, "Init single ok");
ESP_LOGD(TAG, "Init single ok");
    InitializeButtons();
    initializeIot();
      InitializeCamera();
