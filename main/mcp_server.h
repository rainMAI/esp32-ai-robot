...
// 添加类型别名
using ReturnValue = std::variant<bool, int, long long, std::string>;
...
private:
    std::string name_;
    PropertyType type_;
    std::variant<bool, int, long long, std::string> value_;
    ...
    inline T value() const {
        if constexpr (std::is_same_v<T, long long>) {
            if (std::holds_alternative<int>(value_)) {
                return (long long)std::get<int>(value_);
            }
        } else if constexpr (std::is_same_v<T, int>) {
            if (std::holds_alternative<long long>(value_)) {
                return (int)std::get<long long>(value_);
            }
        }
