...
void McpServer::AddCommonTools() {
    ...
    AddTool("self.reminder.add",
        ...
        "1. 用户说类似'10点59提醒我喝水'\n"
        "2. 提取时间: '10:59' 和内容: '喝水'\n"
        "3. 调用此工具: reminder.add(content='喝水', time='10:59')\n"
        "\n"
        "示例：\n"
        ...
        "- 用户: '22点30提醒我去散步' → 调用: reminder.add(content='去散步', time='22:30')\n"
        "\n"
        ...
        [](const PropertyList& properties) -> ReturnValue {
            ...
            ESP_LOGI(TAG, "Adding reminder with time: %s, content: %s", time_str.c_str(), content.c_str());

            // 直接将时间和内容传给 ReminderManager，让它发送到服务器
            bool success = ReminderManager::GetInstance().AddReminderWithTime(content, time_str);
            if (success) {
    ...
    // 聊天记录上传工具
    AddTool("self.chat.upload_batch",
        "Upload a batch of chat messages (user and AI dialogues) to the server for daily report generation.\n"
        "This tool is called automatically when the dialogue buffer reaches the threshold or on a timer.\n"
        "Parameters:\n"
        "- data: JSON array of chat messages, each containing 'user_text' and 'ai_text'\n"
        "Returns: true on success, or error message JSON on failure.",
        PropertyList({
            Property("data", kPropertyTypeString)
        }),
        [](const PropertyList& properties) -> ReturnValue {
            std::string data_json = properties["data"].value<std::string>();

            if (data_json.empty()) {
                ESP_LOGE(TAG, "Chat data is empty");
                return "{\"success\": false, \"message\": \"聊天数据不能为空\"}";
            }

            ESP_LOGI(TAG, "Uploading chat batch via MCP: %s", data_json.c_str());

            // Use EspNetwork to send HTTP POST to web_server
            EspNetwork network;
            ...
            // Send the data
            http->Write(data_json.c_str(), data_json.length());

            int status_code = http->GetStatusCode();
            if (status_code != 200 && status_code != 201) {
                ESP_LOGE(TAG, "HTTP chat upload failed with status: %d", status_code);
                std::string response = http->ReadAll();
                http->Close();
                return std::string("{\"success\": false, \"message\": \"上传失败, 状态码: ") +
                       std::to_string(status_code) + std::string("\"}");
            }

            std::string response = http->ReadAll();
            http->Close();

            ESP_LOGI(TAG, "Chat upload response: %s", response.c_str());

            // Parse response to verify success
            cJSON* root = cJSON_Parse(response.c_str());
            if (root == nullptr) {
                ESP_LOGE(TAG, "Failed to parse chat upload response");
                return "{\"success\": false, \"message\": \"服务器响应解析失败\"}";
            }

            cJSON* success = cJSON_GetObjectItem(root, "success");
            bool is_success = (success != nullptr && cJSON_IsTrue(success));

            cJSON_Delete(root);

            if (is_success) {
                ESP_LOGI(TAG, "Chat batch uploaded successfully via MCP");
                return true;
            } else {
                ESP_LOGE(TAG, "Chat batch upload failed");
