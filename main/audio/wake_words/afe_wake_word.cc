...
bool AfeWakeWord::Initialize(AudioCodec* codec) {
    ...
    // 注意：ESP-IDF 5.4.1 中 afe_config_t 没有 det_threshold 成员
    // 检测阈值由 WakeNet 模型内部决定（通常为 0.6-0.7）
    ESP_LOGI(TAG, "WakeWord detection initialized (threshold controlled by model)");
...
void AfeWakeWord::Feed(const std::vector<int16_t>& data) {
    ...
    if (!(bits & DETECTION_RUNNING_EVENT)) {
        ...
        //     ESP_LOGW(TAG, "WakeWord detection stopped, not feeding data");
        //     last_log = now;
        // }
        return;
    }
...
void AfeWakeWord::AudioDetectionTask() {
    ...
    while (true) {
        ...
        if (res->wakeup_state == WAKENET_DETECTED) {
            ESP_LOGI(TAG, "⭐ WakeWord DETECTED! Word: %s",
                     wake_words_[res->wakenet_model_index - 1].c_str());

            // 使用内存监控工具记录事件
            char detail[128];
            snprintf(detail, sizeof(detail), "唤醒词: %s", wake_words_[res->wakenet_model_index - 1].c_str());
            MemoryMonitor::LogEvent(MEM_EVENT_WAKE_WORD, detail);
