...
bool ReminderManager::AddReminderWithTime(const std::string& content, const std::string& time_str) {
    ESP_LOGI(TAG, "Adding reminder with time string: %s, content: %s", time_str.c_str(), content.c_str());

    // 解析时间字符串，格式如 "10:59" 或 "22:50"
    ...
    // 使用现有的 AddRemote 方法发送到服务器
    if (!AddRemote(content, target_timestamp)) {
        ESP_LOGE(TAG, "Failed to add reminder to server");
        return false;
    }
...
bool ReminderManager::SyncPull(const std::string& server_url) {
    ...
    if (device_settings != nullptr) {
        ...
        if (timezone != nullptr && cJSON_IsString(timezone)) {
            std::string server_tz = timezone->valuestring;
            ESP_LOGI(TAG, "[TZ] Server returned timezone: %s", server_tz.c_str());

            // 转换时区格式："+08:00" → "UTC-8"
            std::string posix_tz;
            if (server_tz.length() >= 4) {
                // 解析格式如 "+08:00" 或 "-07:00"
                char sign = server_tz[0];
                int hours = std::stoi(server_tz.substr(1, 2));

                // POSIX 格式符号相反：UTC-8 = 东8区
                int posix_sign = (sign == '+') ? -1 : 1;
                char tz_buf[20];
                snprintf(tz_buf, sizeof(tz_buf), "UTC%+d", posix_sign * hours);
                posix_tz = std::string(tz_buf);

                ESP_LOGI(TAG, "[TZ] Converting '%s' to POSIX format '%s'", server_tz.c_str(), posix_tz.c_str());
                ...
                ESP_LOGI(TAG, "[TZ] Local time after timezone change: %s", time_buf);
            }
        }
    }
