...
### 新增 (Added)
- **每日AI对话记录功能**：自动记录用户-AI对话（STT用户输入 + TTS AI回复）
- **批量对话上传**：支持批量上传到服务器（默认2条对话或10分钟触发）
- **服务器端AI报告生成**：每天23:00自动生成AI思考报告（思考热力图、概念网络、思考勋章、明日挑战）
- **MCP对话上传工具**：新增 `self.chat.upload_batch` MCP工具，支持AI助手主动触发对话上传
- **AI回复累积机制**：支持多个TTS句子合并为完整AI回复

### 优化 (Improved)
- **HTTP传输优化**：使用SetContent()方法替代Write()，避免分块传输编码问题
- **JSON格式统一**：统一批量上传JSON格式为 `{"messages": [...]}`
- **设备识别优化**：基于MAC地址进行设备识别
- **线程安全优化**：ChatRecorder添加mutex保护，确保多线程安全

### 修复 (Fixed)
- **格式字符串兼容性**：修复ESP-IDF日志系统不支持%zu的问题，改为%d并强制类型转换
- **HTTP传输编码问题**：解决Content-Length与分块传输编码冲突问题
- **AI文本累积逻辑**：修复多个TTS句子无法正确累积的问题

### 技术细节
- ChatRecorder单例模式管理对话记录
- 支持NVS持久化（掉电保护）
- JSON格式：`{"messages": [{"user_text": "...", "ai_text": "..."}]}`
- 服务器端使用DeepSeek API生成报告
- 数据存储在SQLite数据库（face_analysis.db）
