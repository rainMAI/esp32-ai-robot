...
uint32_t gfx_render_area_summary(gfx_core_context_t *ctx)
{
    ...
    for (uint8_t i = 0; i < ctx->disp.dirty_count; i++) {
        ...
        ESP_LOGD(TAG, "Draw area [%u]: (%d,%d)->(%d,%d) %dx%d",
                 i, (int)area->x1, (int)area->y1, (int)area->x2, (int)area->y2,
                              ...
                              uint8_t area_idx, uint32_t start_block_count)
{
    ...
    if (per_flush == 0) {
        ESP_LOGE(TAG, "Area[%u] width %lu exceeds buffer width, skipping", area_idx, area_width);
        return 0;
    }
