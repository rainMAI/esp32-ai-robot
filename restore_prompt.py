...
# 恢复原来的 prompt（去掉开头的提醒规则）
original_prompt = '''我是一个叫小宇同学的AI导师。
...
重要：设置提醒必须调用 self.reminder.add 工具，不能只是文字回复！'''

conn = sqlite3.connect('/opt/xiaozhi-esp32-server-golang/manager/backend/data/xiaozhi.db')
cursor = conn.cursor()
cursor.execute("UPDATE agents SET custom_prompt = ? WHERE id=1", (original_prompt,))
conn.commit()
print("Prompt restored successfully")
conn.close()
